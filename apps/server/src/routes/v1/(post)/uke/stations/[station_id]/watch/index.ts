...
async function handler(req: FastifyRequest<ReqParams>, res: ReplyPayload<JSONBody<ResponseData>>) {
  ...
  await db
    .insert(ukeStationWatches)
    .values({
      userId: session.user.id,
      ukeStationId: req.params.station_id,
    })
    .onConflictDoNothing({ target: [ukeStationWatches.userId, ukeStationWatches.ukeStationId] });

  return res.send({ data: { ok: true } });
}

const watchUkeStation: Route<ReqParams, ResponseData> = {
...
export default watchUkeStation;
