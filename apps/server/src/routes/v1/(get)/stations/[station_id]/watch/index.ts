...
async function handler(req: FastifyRequest<ReqParams>, res: ReplyPayload<JSONBody<ResponseData>>) {
  ...
  const watch = await db.query.stationWatches.findFirst({
    where: {
      AND: [{ userId: { eq: session.user.id } }, { stationId: { eq: req.params.station_id } }],
    },
    columns: { id: true },
  });
...
const getStationWatch: Route<ReqParams, ResponseData> = {
