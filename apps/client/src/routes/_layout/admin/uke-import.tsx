...
function JobStateIndicator({ state }: { state: JobState }) {
	switch (state) {
		case "running":
			return <span className="inline-flex items-center gap-1.5 text-sm font-medium text-blue-500"><Spinner className="size-3.5" /></span>;
		case "success":
			return <HugeiconsIcon icon={CheckmarkCircle02Icon} className="size-4 text-green-500" />;
		case "error":
					...
					<div className="rounded-lg border bg-card p-4">
						...
						<div className="space-y-3">
							...
							<label htmlFor="import-radiolines" className="flex items-center gap-2 cursor-pointer">
								<Checkbox id="import-radiolines" checked={importRadiolines} onCheckedChange={(val) => setImportRadiolines(!!val)} disabled={isRunning} />
						...
						<div className="rounded-lg border bg-card p-4">
							<div className="flex items-center gap-2 mb-4">
								...
								<h3 className="text-sm font-semibold leading-none">
									{t(`ukeImport.status.${status.state}`)}
								</h3>
							</div>

							{status.startedAt && (
								<p className="text-xs text-muted-foreground mb-1">
									{t("ukeImport.startedAt")}: {new Date(status.startedAt).toLocaleString()}
								</p>
							)}
							{status.finishedAt && (
								<p className="text-xs text-muted-foreground mb-1">
									{t("ukeImport.finishedAt")}: {new Date(status.finishedAt).toLocaleString()}
								</p>
							)}
									...
									{status.steps.map((step: ImportStep) => (
										...
										>
											...
											<span className="text-xs text-muted-foreground">
												{t(`ukeImport.stepStatus.${step.status}`)}
											</span>
										</div>
									))}
								</div>
