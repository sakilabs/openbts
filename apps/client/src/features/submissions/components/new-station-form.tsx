			...
			<div className="p-4 space-y-4">
				<div className="grid grid-cols-2 gap-3">
					<div className="space-y-1.5">
						<Label htmlFor="station_id" className="text-xs">{t("newStation.stationId")}</Label>
						<Input
							id="station_id"
					...
					<div className="space-y-1.5">
						<Label htmlFor="operator" className="text-xs">{t("newStation.operator")}</Label>
						<Select
											...
											<div
												...
												style={{
													backgroundColor: getOperatorColor(
														operators.find((o) => o.id === station.operator_id)?.mnc ?? 0,
													),
												}}
											/>
						...
						{errors?.operator_id && (
							<p className="text-xs text-destructive">{t(errors.operator_id)}</p>
						)}
					</div>
				</div>

				<div className="space-y-1.5">
					<Label htmlFor="notes" className="text-xs">{t("newStation.notes")}</Label>
					<Textarea
						id="notes"
