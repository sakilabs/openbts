...
export function BackendStatusProvider({ queryClient, children }: { queryClient: QueryClient; children: ReactNode }) {
	...
	useEffect(() => {
		...
		const unsubscribe = cache.subscribe((event) => {
			if (event.type === "updated" && event.action.type === "error" && event.action.error instanceof BackendUnavailableError)
	...
	if (isUnavailable)
		return <BackendUnavailableScreen onRetry={() => window.location.reload()} />;
