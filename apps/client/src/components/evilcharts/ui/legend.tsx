...
function ChartLegendContent({
  ...
  return (
    <div
      className={cn(
        "flex w-full max-w-full flex-wrap items-center gap-x-4 gap-y-1 select-none",
    ...
    >
      {payload
        ...
        .map((item) => {
          ...
          const keySource = payloadName ?? item.value ?? item.dataKey;
          const key = typeof keySource === "string" || typeof keySource === "number" ? String(keySource) : "value";
          ...
          return (
            <div
              key={key}
              className={cn(
                "[&>svg]:text-muted-foreground flex min-w-0 items-center gap-1.5 transition-opacity [&>svg]:h-3 [&>svg]:w-3",
            ...
            >
              {itemConfig?.icon && !hideIcon ? <itemConfig.icon /> : <LegendIndicator variant={variant} dataKey={key} colorsCount={colorsCount} />}
              <span className="min-w-0 max-w-28 leading-tight sm:max-w-none">{itemConfig?.label}</span>
            </div>
          );
        })}
    </div>
...
function LegendIndicator({ variant, dataKey, colorsCount }: { variant: ChartLegendVariant; dataKey: string; colorsCount: number }) {
